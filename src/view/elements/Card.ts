...
export function Card(props) {
  ...
  // @ts-expect-error TS(7031) FIXME: Binding element 'node' implicitly has an 'any' typ... Remove this comment to see the full error message
    ...
    const el = document.createElementNS('http://www.w3.org/2000/svg', 'g'),
      ...
      card_body = () =>
        !d.data.to_add
          ...
          : CardBodyAddNew({ d, card_dim, card_add: props.cardEditForm, label: unknown_lbl }).template,
      card_image = () =>
        !d.data.to_add
  ...
  function setupSvgDefs() {
    ...
    props.svg.insertAdjacentHTML(
      ...
      <defs id="f3CardDef">
        ...
        <clipPath id="card_image_clip"><path d="M0,0 Q 0,0 0,0 H${card_dim.img_w} V${card_dim.img_h} H0 Q 0,${
        card_dim.img_h
      } 0,${card_dim.img_h} z"></clipPath>
        <clipPath id="card_image_clip_curved"><path d="${curvedRectPath({ w: card_dim.img_w, h: card_dim.img_h }, 5, [
          'rx',
          'ry',
        ])}"></clipPath>
      </defs>
    ...
    function curvedRectPath(dim, curve, no_curve_corners) {
      const { w, h } = dim,
        ...
        // @ts-expect-error TS(7006) FIXME: Parameter 'corner' implicitly has an 'any' type.
